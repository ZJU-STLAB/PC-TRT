#ifndef PCTRT_TESTGENERATOR_H
#define PCTRT_TESTGENERATOR_H
...
public:
    ...
    bool run(){
        ...
        auto driverFile = kleeDriverGenerator.generate(functionName, DRIVER_TYPE::DRIVER_KLEE_SE);
        if(driverFile.empty()){
            // 手动输入驱动函数文件名
            std::cout << "Generate driver file failed, please input klee diver file name:" << std::endl;
            std::cin >> driverFile;
            if(!fileExists(driverFile)){
                std::cerr << "Driver file does not exist." << std::endl;
                exit(1);
            }
        }
        // 将驱动函数文件编译为LLVM IR
        ...
        PCTRT_ASSERT(compiled, "Failed to compile driver file.");
        // 对LLVM IR插桩
