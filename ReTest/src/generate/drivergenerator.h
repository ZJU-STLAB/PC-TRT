#ifndef PCTRT_DRIVERGENERATOR_H
#define PCTRT_DRIVERGENERATOR_H
...
public:
    ...
    std::string generate(std::string functionName, DRIVER_TYPE driverType){
        // 修改源文件中的main函数
    ...
    std::vector<std::pair<std::string, std::string>> getFunctionParameters(const std::string& functionDeclaration, const std::string functionName){
        ...
        PCTRT_ASSERT(paraStart != std::string::npos && paraEnd != std::string::npos, "Cannot find function parameters");
    ...
    std::string generateExecutableDriver(std::string functionName){
    ...
    std::string generateKleeDriver(std::string functionName) {
        auto functionDeclaration = getFunctionDeclaration(srcFileName, functionName);
        if(functionDeclaration.empty()){
            std::cerr << "Cannot find function declaration for " << functionName << std::endl;
            return {};
