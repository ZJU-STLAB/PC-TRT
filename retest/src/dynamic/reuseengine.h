#ifndef RETEST_REUSEENGINE_H
#define RETEST_REUSEENGINE_H
...
public:
    ...
    virtual double calculate(const Path& path1, const Path& path2) = 0;
    virtual double getNodeSimilarity(const Node* node1, const Node* node2) = 0;
};

// 利用基本块序列的指令类型来做相似度计算
class NormalStrategy : public SimilarityStrategy {
private:
    std::unordered_map<int, std::unordered_map<int, double>> sim_cache;

public:
    double calculate(const Path& path1, const Path& path2) final {
        size_t m = path1.size();
        size_t n = path2.size();
        double dp[m + 1][n + 1];
        for(int i = 0; i <= m; ++i){
    ...
    double getNodeSimilarity(const Node* node1, const Node* node2) final {
        RETEST_ASSERT(node1 != nullptr && node2 != nullptr, "node1 or node2 is nullptr");
...
// 利用数据流来做相似度计算
class FlowStrategy : public SimilarityStrategy {
    double calculate(const Path& path1, const Path& path2) final {
        return 0;
    }
    double getNodeSimilarity(const Node* node1, const Node* node2) final {
...
public:
    ...
    void setStrategy(const std::string& strategy_name){
        ...
        }else if(strategy_name == "flow") {
            strategy = std::make_unique<FlowStrategy>();
        }
    }
...
private:
    ...
    std::pair<int, double> findMostSimilarPath(const Path& path){
        ...
        for (const auto& old_path : old_paths) {
            RETEST_ASSERT(strategy != nullptr, "calculate strategy is nullptr");
...
public:
    ...
    void init() {
        ...
        DriverGenerator driverGenerator(oldSrcFile);
        driverGenerator.generate(funcName, DRIVER_TYPE::DRIVER_EXECUTABLE);
        auto driverFile = getDirPath(oldSrcFile) + getBaseName(oldSrcFile) + "_driver.c";
        compileDriverAndInstrument(driverFile, funcName);
        // 2. 编译新版本的源文件并添加驱动函数
        auto newIrFile = getDirPath(newSrcFile) + getBaseName(newSrcFile) + ".ll";
        compileSrcToIR(newSrcFile, newIrFile);
        DriverGenerator newDriverGenerator(newSrcFile);
        newDriverGenerator.generate(funcName, DRIVER_TYPE::DRIVER_EXECUTABLE);
        auto newDriverFile = getDirPath(newSrcFile) + getBaseName(newSrcFile) + "_driver.c";
        compileDriverAndInstrument(newDriverFile, funcName);
    }
