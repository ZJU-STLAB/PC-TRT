...
namespace std {
    template <>
    struct ::std::hash<retest::src_loc> {
        size_t operator()(const retest::src_loc &pos) const {
...
public:
    explicit Path(int total_nodes, const std::vector<const Node*>& nodes) :
        id_(count_++), total_nodes(total_nodes), mask(std::make_unique<pathMask>(total_nodes)) {
            ...
            for(auto node : nodes){
                ...
                RETEST_ASSERT(node_id >= 0 && node_id < total_nodes, "Node id is out of range.");
                mask->setBit(node_id);
            }
    }
    ...
    [[nodiscard]] const Node* getNode(int idx) const {
        RETEST_ASSERT(idx >= 0 && idx < nodes.size(), "Index is out of range.");
        return nodes[idx];
    }
    ...
    struct pathMask {
        ...
        void setBit(int index) const {
            RETEST_ASSERT(index >= 0 && index < numNodes, "Index is out of range.");
            nodeMask[index] = true;
        }

        [[nodiscard]] bool isCover(const pathMask& other) const {
//            RETEST_ASSERT(numNodes == other.numNodes, "The number of nodes is not equal.");
...
public:
    ...
    Path& getPath(int idx){
        RETEST_ASSERT(idx >= 0 && idx < paths.size(), "Index is out of range.");
        return paths[idx];
    }
    ...
    std::vector<int> getBlockSuccessors(int blockId){
        RETEST_ASSERT(blockId >= 0 && blockId < size, "Block id is out of range.");
        return edges[blockId];
    }

    void initGraphFromFunction(llvm::Function* function){
        RETEST_ASSERT(function != nullptr && !function->empty(), "Function cannot be empty!");
        func = function;
        size = function->size();
        nodes.reserve(size);
        edges.resize(size);
        ...
        for (auto& block : *function) {
            ...
            node_map[&block] = bbId;
            RETEST_ASSERT(bbId == nodes.back().getId(), "Node id doesn't match!");
            bbId++;
        }
    ...
    std::string getSrcWithLoc(const src_loc& loc){
        RETEST_ASSERT(srcLocMap.count(loc) > 0, "Cannot find the location in source file.");
        std::string ret;
